...
| **Testing** | Deno Test + Docker PostgreSQL | Real PostgreSQL via `docker compose` (test DB: `app_test`) |
| **Deploy** | [Deno Deploy](https://deno.com/deploy) | Edge-native hosting via GitHub Actions + `DENO_DEPLOY_TOKEN` |
...
| `deno task --filter '@app/api' db:push:test` | Push schema to test DB |
...
2. Add `DENO_DEPLOY_TOKEN`:
   - Create it in the Deno dashboard: **dash.deno.com → Account Settings → Access Tokens**
   - Use a token that can deploy to your target project
3. In **Settings > Secrets and variables > Actions > Variables**, add `DENO_DEPLOY_PROJECT` with your Deno Deploy project name
...
- **API tests** use **Docker PostgreSQL** — run `docker compose up -d` and `deno task --filter '@app/api' db:push:test`
